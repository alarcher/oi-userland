    ...
    def __init__(self, path=None, debug=False):
        ...
        if path:
            ...
            if not os.path.isfile(component_pkg5_file):
                # get component name
                component_name = self.run_make(
                    path, 'print-value-COMPONENT_NAME')
                if not component_name:
                    raise ValueError(
                        'Component returns empty name at ' + self.path + '.')
                else:
                    self.name = component_name[0]
                # get supplied packages, this may trigger the creation of a pkg5.fmris file
                self.supplied_packages = self.run_make(
                    path, 'print-package-names')
                # always update fmris if list is overriden
                component_pkg5_fmris_file = os.path.join(
                    self.path, 'pkg5.fmris')
                ...
                # get dependencies
                self.required_packages = self.run_make(
                    path, 'print-required-packages')
                ...
                if not data:
                    raise ValueError(
                        'Component pkg5 data is empty for path ' + self.path + '.')
                self.name = data['name']
